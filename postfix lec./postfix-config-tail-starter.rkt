...
(define (postfix-exec-config-tail cmds stk)
  (cond ((null? cmds) (first stk))
        ((eq? (first cmds) 'exec) (postfix-exec-config-tail
                                   (append (first stk) (rest cmds)) (rest stk)))
...
(define (postfix-exec-command cmd stk)
  (cond ((integer? cmd) (cons cmd stk))
        ...
        ((eq? cmd 'mul)
         (cons (* (second stk) (first stk)) (rest (rest stk))))
        ((eq? cmd 'div)
         (cons (quotient (second stk) (first stk)) (rest (rest stk))))
        ((eq? cmd 'lt)
        ...
        ((eq? cmd 'nget)
         (let* {[index (first stk)]
                [val (list-ref stk index)]}
           (if (not (integer? val))
               (error "nget can't return noninteger" val) ; turning complete to not turning complete
           (cons val (rest stk)))))
        ((postfix-command-sequence? cmd)
         (cons cmd stk))
