...
(define (postfix-exec-command cmd stk)
  (cond ((integer? cmd) (cons cmd stk))
        ...
        ((eq? cmd 'dup)
         (if (< (length stk) 1)
             (error "ERROR: dup requires a nonempty stack" (list cmd stk))
             (cons (first stk) stk)))
        ...
        ((eq? cmd 'rot)
         (if (< (length stk) 1)
             (error "rot requires nonempty stack" stk)
             (let {[len (first stk)]}
               (cond ((or (not (integer? len))  (<= len 0))
                      (error "rot length must be a positive integer but is" len))
                     ((< (length stk) (+ len 1))
                      (error "not enough stack values for rot" stk))
                     (else
                      (let {[newlst (rest (rest stk))]
                            [v1 (second stk)]}
                        (append (list-head newlst (- len 1))
                                (cons v1 (list-tail newlst (- len 1))))))))))
...
(define postfix-arithops
  (list (list 'add +)
        ...
        (list 'chs (λ (a b)
                     (if (or (< a 0) (< b 0) (> b a))
                         (error "ERROR: invalid operands for chs" (list a b))
                         (quotient (fact-iter a)
                                   (* (fact-iter b) (fact-iter (- a b)))))))
        ))

;; Helper function for chs
(define (fact-iter n)
  (fact-tail n 1))
...
(define postfix-relops
  (list (list 'lt <)
        ...
        (list 'gt >)
        ;;----------------------------------------------------------------------
        (list 'le (λ (a b) (or (< a b) (= a b))))
        (list 'ge (λ (a b) (or (> a b) (= a b))))            
        (list 'and (λ (a b) (not (or (= a 0) (= b 0)))))
        ))
...
(define pfg
  '(postfix 3 ; let's call the arguments a, b and c, from top down
            1 nget ; duplicate a at top of stack
            2 rem ; check if a is even
            (swap ; else clause - stack now has b on top and a on second
             3 nget ; duplicate c at top of stack
             add ; add b and c
             mul) ; multiply result by a
            (3 nget ; then clause - duplicate c at top of stack
               sub ; subtract a from c
               div) ; divide b by result from above         
            sel ; if a is even -> then clause, otherwise -> else clause          
            exec ; execute executable sequence based on sel
            sub ; subtract result from c and return 
            ))
